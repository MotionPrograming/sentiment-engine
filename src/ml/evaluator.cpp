...
EvaluationResult Evaluator::evaluate(const vec<Sentiment>& actual, const vec<Sentiment>& predicted) {
    EvaluationResult result{};
    if (actual.empty() || actual.size() != predicted.size()) return result;

    constexpr sz class_count = 3;
    u64 correct = 0;

    for (sz i = 0; i < actual.size(); ++i) {
        const sz actual_class = static_cast<sz>(actual[i]);
        const sz predicted_class = static_cast<sz>(predicted[i]);

        if (actual_class >= class_count || predicted_class >= class_count) continue;

        ++result.confusion[actual_class][predicted_class];
        if (actual_class == predicted_class) ++correct;
    }

    result.total_samples = actual.size();
    result.correct_predictions = correct;
    result.accuracy = static_cast<double>(correct) / static_cast<double>(actual.size());
    ...
    if (total_support > 0) {
        result.weighted_f1 = weighted_f1_sum / static_cast<double>(total_support);
    }
