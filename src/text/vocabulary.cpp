...
Vocabulary::TokenId Vocabulary::add(sv token) {
...
Vocabulary::TokenId Vocabulary::find(sv token) const noexcept {
    auto it = token_to_id_.find(std::string(token));
...
Vocabulary::TokenId Vocabulary::find(const str& token) const noexcept {
...
const str& Vocabulary::token(TokenId id) const {
    return id_to_token_.at(id);
}

const str& Vocabulary::term(TokenId id) const {
    return token(id);
}
